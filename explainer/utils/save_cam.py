...
def save_cam(result_output_folder, final_out_cam, pid, study_uid, series_uid):
    """
    ...
    uid = determine_cam_save_name(pid, study_uid, series_uid)
    uid = uid.replace("'", "").replace("[", "").replace("]", "").replace("/", "_")
    print(f"Saving CAM to {os.path.join(result_output_folder, uid)}")

    np.savez_compressed(
        os.path.join(result_output_folder, uid), cam=final_out_cam
    )
...
def save_to_dicom(images, dcms, target_folder, _id_str=".1111", series_number=111):
    ...
    for index, (ds) in enumerate(dcms):
        ...
        ds.WindowCenter = 120
        ds.WindowWidth = 230
        ds.SeriesInstanceUID = ds.SeriesInstanceUID + _id_str
        ds.SOPInstanceUID = ds.SOPInstanceUID + _id_str
